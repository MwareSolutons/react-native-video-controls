...
export default class VideoPlayer extends Component {
    ...
    constructor( props ) {
        ...
        this.events = {
            ...
            onTextTrackSelection: this.props.onTextTrackSelection || this._onTextTrackSelection.bind( this ),
            onAudioTrackSelection: this.props.onAudioTrackSelection || this._onAudioTrackSelection.bind( this ),
            onScreenSize: this.props.onScreenSize || this._onScreenSize.bind( this ),
         ...
         */
            ...
            this.player = {
                ...
                iconOffset: 7,
                seekWidth: 0,
                ref: this.state.playertype == 'vlc' ? VLCPlayer : Video
            };
    ...
    | -------------------------------------------------------
    |
    | These are the events that the v component uses
    ...
    */
    ...
    _onProgress( data = {} ) {
        let state = this.state;
        if(this.props.playertype == 'vlc'){
            state.duration = (data.duration / 60000) * 60;
            state.currentTime = data.position * 1000;
        }else{
     ...
     *
     * @param {object} err  Err obj returned from v component
     */
    _onError( err ) {
        let state = this.state;
        state.error = true;
    ...
    */
     ...
     * the v component then updates the
     * isFullscreen state.
     */
    _toggleFullscreen() {
        let state = this.state;
    ...
    /**
     * Toggle playing state on v component
     */
    _togglePlayPause() {
        let state = this.state;
    ...
    _onAudioTrackSelection(){

    }
    _onTextTrackSelection(){
    ...
    seekTo( time = 0 ) {
        ...
        if(this.props.playertype == 'vlc'){
            this.player.ref.seek( time / 1000);
        }else{
    ...
    */
    ...
    componentWillReceiveProps(nextProps) {
        if (this.state.paused !== nextProps.paused && this.state.first == false && this.state.second == false ) {
            this.setState({
                paused: nextProps.paused,
                first: true
            })
        } else if (this.state.paused !== nextProps.paused && this.state.first == true && this.state.second == false ) {
            this.setState({
                paused: nextProps.paused,
                second: true
            })
    ...
    */
    ...
    renderBottomControls() {
        ...
        const textTrackControl = this.renderTextTrackSelection();
        const audioTrackControl = this.renderAudioTrackSelection();  
        const screenSizeControl = this.renderScreenSize();
    ...
    render() {
        ...
        //console.log(this.props.playertype)
        return (
            <TouchableWithoutFeedback
                onPress={ this.events.onScreenTouch }
                style={[ styles.player.container, this.styles.containerStyle ]}
            >
                <View style={[ styles.player.container, this.styles.containerStyle, {backgroundColor:'transparent'} ]}>
                {renderIf(this.props.playertype == "vlc")(
                    ...
                    <VLCPlayer
                        ...
                        onVLCProgress={this.events.onProgress}
                        onVLCEnded={this.events.onEnd}
                        onVLCStopped={this.events.onEnd}
                        onVLCPlaying={this.events.onLoad}
                        style={[ styles.player.video, this.styles.videoStyle ]}
                        source={ this.props.source }
                    />
            )}
                
                {renderIf(this.props.playertype == "exo")(
                        <Video
                            { ...this.props }
                            ref={ videoPlayer => this.player.ref = videoPlayer }
